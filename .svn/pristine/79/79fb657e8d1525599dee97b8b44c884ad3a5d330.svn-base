			...
			success:function(data){
				if(data.dealMessageList!=null){
					...
					for(var i=0;i<data.dealMessageList.length;i++){
						tab+='<div id="assign'+data.dealMessageList[i].dealId+'" onclick="getpurDetail('+"'"+data.dealMessageList[i].dealId+"'"+')">';
						tab+='<TABLE class="tab">';
						...
						tab+='<TR><TD></TD>';
						tab+='<TD><span class="redfont">￥'+data.dealMessageList[i].originalPrice+'</span></TD>';
						tab+='<TD align="right">';
